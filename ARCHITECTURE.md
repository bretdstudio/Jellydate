...
- Client network work is one bounded 4 KiB read per display update, capped to the socket's current availability so a seek cannot leave the firmware waiting for the rest of an interrupted media packet. The hardware read timeout is also applied after `open()` so the SDK cannot reset it during connection setup. Complete video packets are reconstructed by the streaming parser and only the newest eligible frame is presented.
