...
The current `0.1` spike proves the end-to-end media path in both the Simulator and on physical Playdate hardware. It has a Playdate-native 2×2 home dashboard with animated one-bit mascot sprites for Continue Watching, Movies, TV Shows, and Recently Added; server-filtered A–Z indexes for movies and TV shows; seamless pagination across every catalog; complete series → seasons → episodes navigation with page-and-selection-preserving back behavior; clear unwatched, in-progress, and completed states that refresh after playback; compact media details with cached, dithered Jellyfin poster artwork, title, context, description, runtime, and resume position; persistent on-device bridge setup; real Jellyfin browsing endpoints; authenticated media input; negotiated ordered-dithered video and mono PCM audio; audio-master A/V synchronization; pause/stop; crank seeking; and playback lifecycle reporting.
...
This reads the bridge token from `.env` without copying it into tracked source. The generated values are now development and first-run fallbacks; once Test & Save succeeds, the Playdate stores its bridge host and token in the app's private data directory and uses them on future launches.
...
In the browser, A opens the selected item or drills into a series and B goes back. Playable rows use an open circle for unwatched, a half-filled circle and progress bar for in progress, and a checked box for completed; series and season rows have no playback marker. On a media-details screen, A starts, resumes, or watches again and B returns to a freshly updated catalog. Opening details fetches one poster through the Bridge at four times its 96×144 display resolution. The Bridge auto-orients it, performs a high-quality Lanczos downscale, restores contrast and edge detail, and applies static-image Atkinson dithering before sending the compact 1,728-byte result. Finished posters persist in a cache keyed by Jellyfin image tag, episodes can inherit series art, and unavailable images receive a native fallback instead of delaying the catalog. Artwork work is skipped whenever video is active, so it cannot alter the tested playback profile. Returning from playback refreshes the item after Jellyfin records the stop position. During playback, A pauses/resumes, B returns to the media details, and the crank scrubs. Slow crank motion moves by seconds; faster motion winds the imaginary reel increasingly quickly. The seek is sent half a second after crank movement stops.
