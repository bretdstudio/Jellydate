...
The current `0.1` spike proves the end-to-end media path in both the Simulator and on physical Playdate hardware. It has a Playdate-native 2×2 home dashboard with Continue Watching, Movies, TV Shows, and Recently Added; a server-filtered A–Z movie index; seamless pagination across every catalog; complete series → seasons → episodes navigation with page-and-selection-preserving back behavior; compact media details with title, context, description, runtime, and resume position; real Jellyfin browsing endpoints; authenticated media input; negotiated ordered-dithered video and mono PCM audio; audio-master A/V synchronization; pause/stop; crank seeking; and playback lifecycle reporting. Persistent keyboard-based bridge setup is the next interface milestone.
